<?php
  ...
  print '<tr><td class="m_separator">№ п/п</td><td class="m_separator" align="center">Наименование услуги</td>
	     ...
	     <td class="m_separator" align="center">Расход за период, м3</td></tr>';

  $today=getdate(); $startday=1;
  if ($_POST["day"]!='') $startday=$_POST["day"];
  if ($_POST["day2"]!='') $endday=$_POST["day2"];
  if ($_POST["month"]!='') $startmon=$_POST["month"];
  if ($_POST["month2"]!='') $endmon=$_POST["month2"];
  if ($_POST["year"]!='') $startyear=$_POST["year"];
  if ($_POST["year2"]!='') $endyear=$_POST["year2"];
  
  $adr=strstr ($name,"ул.");
  $date1=sprintf ("%d%02d%02d000000",$startyear,$startmon,$startday);
  $date2=sprintf ("%d%02d%02d000000",$endyear,$endmon,$endday);
